...
public function index(Request $data){
                ...
                if($checkIfActive){
                    if($checkIfActive->active==1){
                        if(Hash::check($request->password, $existingRecord->password)) {
                            ...
                            ->update($data);
                            if(stripos($existingRecord->user_dp,'https://')!==false){
            	                $file_path=$existingRecord->user_dp;
            	                $small_file_path=$existingRecord->user_dp;
            	            }else{
            	                $file_path = asset(Storage::url('public/profile_pic/'.$existingRecord->user_id."/".$existingRecord->user_dp));
                                $small_file_path = asset(Storage::url('public/profile_pic/'.$existingRecord->user_id."/small/".$existingRecord->user_dp));
                                if($file_path==""){
                            ...
                            $data  = array( 'user_id' => $existingRecord->user_id, 'app_token' => $user_token,'username' =>$existingRecord->username,'email'=>$existingRecord->email,'fname'=>$existingRecord->fname,'lname'=>$existingRecord->lname,'user_dp'=>$file_path );
                        }else{
    ...
    public function socialLogin(Request $request){
        ...
        if($isRecord){
            ...
            $response = array("status" => "success",'msg'=>'Social login successfully' ,'isRecord' => false,'content' => $data); 
        }
