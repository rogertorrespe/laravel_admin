    ...
    //if (messenger != 0) {
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                messageInput.focus();
                // update info in view
                $('.messenger-infoView .info-name').html(data.fetch.fname);
                $('.m-header-messaging .user-name').html(data.fetch.fname);
                // Star status
                data.favorite > 0
...
// While sending a message, show this temporary message card.
function receiveCard(message, id, time) {
    return `
    <div class="message-card" data-id="`+ id + `">
        <p>`+ message + ` <sub title="`+ time + `">`+ time + `</sub></p>
    </div>
    `;
}
...
$(document).ready(function () {
    ...
    clearTimeout(typingTimeout);
        ...
        var socket_url = "https://letmesoul.com:3001";
        var socket = io(socket_url,{transports: ['websocket'], upgrade: false});
        
        window.addEventListener("load", (event) => {
            let statusDisplay = $('.internet-connection');
            statusDisplay.textContent = navigator.onLine ? "connected" : "disconnected";
            checkInternet(statusDisplay.textContent, statusDisplay);
          });
        ...
        socket.on('connect', () => {
            ...
            // IDinfo(socket.id, 'user');
            socket.emit("user-id", auth_id, function (data) {
        ...
        socket.on('user-id', function (data) {
            console.log('ssssss '+data.id);
            setActiveStatus(1, data.id);
            $('.messenger-list-item[data-contact=' + data.id + ']').find('.activeStatus').remove();
            $('.messenger-list-item[data-contact=' + data.id + ']').find('.avatar').before(activeStatusCircle());
        });

        socket.on('disconnect', function (data) {
            setActiveStatus(0, data.id);
            $('.messenger-list-item[data-contact=' + data.id + ']').find('.activeStatus').remove();
        });
        /**
         *-------------------------------------------------------------
        ...
        */
        ...
        socket.on('send_msg', function (data) {
            ...
            if (data.from_id == messenger.split('_')[1] && data.to_id == auth_id && messenger.split('_')[1]!=auth_id) {
                // remove message hint
                $(".message-hint").remove();
                // append message
                temporaryMsgId += 1;
                let tempID = 'temp_' + temporaryMsgId;
                //   console.log('timmmeeeee '+JSON.stringify(data));
                messagesContainer.find('.messages').append(receiveCard(data.msg, tempID, 'just now'));
                // messagesContainer.find('.messages').append(receiveCard(data.msg, tempID, data.sent_on));

                //messagesContainer.find('.messages').append(data.msg);
                // scroll to bottom
                scrollBottom(messagesContainer);
                // trigger seen event
                makeSeen(true);
            ...
            if(auth_id==data.to_id){
                if(data.from_id != messenger.split('_')[1]){
                    var count_msg=0;
                    //alert($('.messenger-list-item[data-contact=' + data.from_id + ']').find('tr td:nth-child(2) ' ).find('b').text());
                    if($('.messenger-list-item[data-contact=' + data.from_id + ']').find('tr td:nth-child(2) ' ).find('b').text()!=""){
                        count_msg= $('.messenger-list-item[data-contact=' + data.from_id + ']').find('tr td:nth-child(2) ' ).find('b').text();
                        count_msg=parseInt(count_msg)+1;
                        $('.messenger-list-item[data-contact=' + data.from_id + ']').find('tr td:nth-child(2) ' ).find('b').text(count_msg);
                    }else{
                        count_msg++;
                        $('.messenger-list-item[data-contact=' + data.from_id + ']').find('tr td:nth-child(2) span:nth-child(2)' ).after('<b>'+count_msg+'</b>');
                    }
                }
                $('.messenger-list-item[data-contact=' + data.from_id + ']').find('tr td:nth-child(2) span:not(.activeStatus)').text(data.msg);
                $('.messenger-list-item[data-contact=' + data.from_id + ']').find('tr td:nth-child(2) p[data-id=user_'+data.from_id+'] span:not(.activeStatus)' ).text(data.time);
                $('.blink_msg').show();
            }
            // $('.mc-sender p sub').find('svg').removeClass('fa-check').addClass('fa-check-double');
        ...
        function isTyping(status) {
            return  socket.emit("typing", { from_id: auth_id,to_id: messenger.split('_')[1], status:status }, function (data) {
                ...
                $.ajax({
                    ...
                    success: (data) => {
                        ...
                        if (data.error > 0) {
                            ...
                            socket.emit("send_msg", { from_id: auth_id,to_id: messenger.split('_')[1], msg: data.msg, sent_on: data.sent_on, time:data.time });
                            // update contact item
